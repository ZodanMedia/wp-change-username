...
function zodan_change_username_get_user_roles() {
	...
	return apply_filters( 'zodan_change_username_user_roles', $roles );
}
...
function zodan_change_username_can_change_own_username() {
	...
	if ( is_user_logged_in() ) {
		$allowed_roles = zodan_change_username()->settings->get_option( 'allowed_roles', array() );
	...
	return apply_filters( 'zodan_change_username_can_change_own_username', $allowed );
}
 ...
 */
function zodan_change_username_process( $old_username, $new_username ) {
	global $wpdb;
	...
	if ( $user_id ) {
		...
		do_action( 'zodan_change_username_before_process', $old_username, $new_username );
		...
		if ( false !== $wpdb->query( $q ) ) { // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,WordPress.DB.DirectDatabaseQuery
			...
			if ( zodan_change_username_plugin_installed( 'co-authors-plus/co-authors-plus.php' ) ) {
		...
		// Let devs hook into the process.
		do_action( 'zodan_change_username_after_process', $old_username, $new_username );
 ...
 */
function zodan_change_username_plugin_installed( $plugin = false ) {
	$ret = false;
